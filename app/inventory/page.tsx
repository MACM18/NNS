...
import { Plus, Package, TrendingDown, AlertTriangle, BarChart3, Eye } from "lucide-react"
...
export default function InventoryPage() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const totalWaste = wasteData?.reduce((sum, item) => sum + (item.quantity || 0), 0) || 0
      const totalStockValue = totalStock?.reduce((sum, item) => sum + (item.current_stock || 0), 0) || 1
                ...
                <CardContent>
                  {loadingData ? (
                        ...
                        {drums.map((drum) => {
                          ...
                          return (
                            <TableRow key={drum.id}>
                              <TableCell className="font-mono text-sm">{drum.drum_number}</TableCell>
                              ...
                              <TableCell>{new Date(drum.received_date).toLocaleDateString()}</TableCell>
                    ...
                    <div className="text-center py-8 text-muted-foreground">
                      <Package className="h-12 w-12 mx-auto mb-4 opacity-50" />
                      <p>No drums found</p>
                      <p className="text-sm">Drums are automatically created when cable items are received</p>
                    </div>
                  )}
                </CardContent>
              </Card>
                ...
                <CardContent>
                  {loadingData ? (
                      ...
                      <TableBody>
                        {wasteReports.map((waste) => (
                          <TableRow key={waste.id}>
                            <TableCell>{new Date(waste.date).toLocaleDateString()}</TableCell>
                            ...
                            <TableCell>
                              {waste.cost_estimate ? `LKR ${waste.cost_estimate.toLocaleString()}` : "N/A"}
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    ...
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-sm">Record waste to track material losses and costs</p>
                    </div>
                  )}
                </CardContent>
              </Card>
