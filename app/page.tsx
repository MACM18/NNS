...
import { Cable, CheckCircle, AlertCircle, TrendingUp, Plus, ArrowRight } from "lucide-react"
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const { data: previousLines } = await supabase
        .from("telephone_lines")
        .select("*")
        .lte("created_at", `${previousYear}-${previousMonth.toString().padStart(2, "0")}-31`)
      ...
      const { data: currentInvoices } = await supabase
        ...
        .select("total_amount")
        .gte("job_month", `${currentYear}-${currentMonth.toString().padStart(2, "0")}-01`)
        .lte("job_month", `${currentYear}-${currentMonth.toString().padStart(2, "0")}-31`)

      const { data: previousInvoices } = await supabase
        .from("generated_invoices")
        .select("total_amount")
        .gte("job_month", `${previousYear}-${previousMonth.toString().padStart(2, "0")}-01`)
        .lte("job_month", `${previousYear}-${previousMonth.toString().padStart(2, "0")}-31`)
                ...
                <Card key={index}>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {loadingStats ? <div className="h-8 w-20 bg-gray-200 animate-pulse rounded"></div> : stat.value}
                    </div>
