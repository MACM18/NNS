            ...
            <Tabs defaultValue="posts" className="space-y-4">
              <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <Button onClick={() => setShowAddPostModal(true)} className="flex-1 sm:flex-none">
                    <Plus className="mr-2 h-4 w-4" />
                    Add Post
                  </Button>
                  <Button onClick={() => setShowAddBlogModal(true)} className="flex-1 sm:flex-none">
                    <Plus className="mr-2 h-4 w-4" />
                    Add Blog
                  </Button>
                </div>
                                ...
                                <TableCell className="font-medium">
                                  <div className="flex flex-col gap-1">
                                    ...
                                    <div className="sm:hidden text-xs text-muted-foreground">
                                      {post.author} • {post.category || "Uncategorized"}
                                    </div>
                                  </div>
                                </TableCell>
                                <TableCell className="hidden sm:table-cell">{post.author}</TableCell>
                                <TableCell className="hidden md:table-cell">{post.category || "Uncategorized"}</TableCell>
                                <TableCell>
                                  <Badge variant={post.status === "active" ? "default" : "secondary"}>{post.status}</Badge>
                                </TableCell>
                                <TableCell className="hidden lg:table-cell">{new Date(post.created_at).toLocaleDateString()}</TableCell>
                                <TableCell>
                                  <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                          ...
                          <TableBody>
                            {filteredBlogs.map((blog) => (
                              <TableRow key={blog.id}>
                                ...
                                <TableCell className="hidden lg:table-cell">{blog.published_at ? new Date(blog.published_at).toLocaleDateString() : "Draft"}</TableCell>
                                <TableCell>
                                  <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                      <Button variant="ghost" size="sm">
