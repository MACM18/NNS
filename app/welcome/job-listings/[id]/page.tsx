...
async function fetchJobVacancy(id: string): Promise<JobVacancy | null> {
  ...
  const { data, error } = await supabase
    .from("job_vacancies")
    .select("id, title, description, location, employment_type, salary_range, created_at, end_date")
    .eq("id", id)
    .single()
      ...
      <PublicLayout>
        <section className="py-12 md:py-24 lg:py-32 text-center">
          <div className="container px-4 md:px-6">
            <h1 className="text-3xl font-bold tracking-tighter sm:text-5xl">Job Not Found</h1>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription className="flex flex-wrap items-center gap-4 mt-2 text-muted-foreground">
                ...
                <span className="flex items-center gap-1">
                  <DollarSign className="h-4 w-4" /> {job.salary_range || "Negotiable"}
                </span>
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="grid gap-2 text-sm text-muted-foreground">
                ...
                {job.end_date && (
                  <div className="flex items-center gap-2">
                    <CalendarDays className="h-4 w-4" />
                    <span>Application Deadline: {format(new Date(job.end_date), "MMM dd, yyyy")}</span>
                  </div>
                )}
              </div>
