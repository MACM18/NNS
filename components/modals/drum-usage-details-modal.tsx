...
import { Cable, AlertTriangle, TrendingDown, Settings, Save, RotateCcw } from "lucide-react"
import { calculateSmartWastage, calculateLegacyWastage, validateManualWastage, type DrumUsage, type WastageCalculationResult } from "@/lib/drum-wastage-calculator"
...
export function DrumUsageDetailsModal({ 
  ...
  const [calculatedMetrics, setCalculatedMetrics] = useState<WastageCalculationResult>({
    totalUsed: 0,
    totalWastage: 0,
  ...
  const [calculationMethod, setCalculationMethod] = useState<'smart_segments' | 'legacy_gaps' | 'manual_override'>('smart_segments')
  ...
  const fetchDrumUsage = async () => {
    ...
    try {
      ...
      const { data: drum, error: drumError } = await supabase
        .from("drum_tracking")
        .select("initial_quantity, current_quantity, manual_wastage_override, wastage_calculation_method")
        .eq("id", drumId)
        .single()
      ...
      recalculateMetrics(usage || [], drum, currentMethod, drum.manual_wastage_override)
    } catch (error: any) {
      addNotification({
        title: "Error",
  ...
  const handleCalculationMethodChange = (method: 'smart_segments' | 'legacy_gaps' | 'manual_override') => {
  ...
  const handleManualWastageChange = (value: string) => {
    ...
    if (calculationMethod === 'manual_override') {
      ...
      const validation = validateManualWastage(numValue, calculatedMetrics.totalUsed, drumInfo.initial_quantity)
  ...
  const saveWastageSettings = async () => {
    ...
    try {
      ...
      if (calculationMethod === 'manual_override') {
        const manualValue = parseFloat(manualWastageInput) || 0
        const validation = validateManualWastage(manualValue, calculatedMetrics.totalUsed, drumInfo.initial_quantity)
      ...
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <DialogDescription>
                Advanced usage tracking with configurable wastage calculation methods
              </DialogDescription>
            </div>
            <Button
              variant="outline"
          ...
          <Card className="border-dashed border-blue-200 bg-blue-50/50 dark:bg-blue-950/50">
            ...
            <CardContent className="space-y-4">
                ...
                <Select value={calculationMethod} onValueChange={handleCalculationMethodChange}>
                  ...
                  <SelectContent>
                    <SelectItem value="smart_segments">
                      Smart Segments (Recommended) - Handles overlapping usage intelligently
                    </SelectItem>
              ...
              {calculationMethod === 'manual_override' && (
                <div>
                  <Label htmlFor="manual-wastage" className="text-sm font-medium">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="text-xs text-muted-foreground mt-1">
                {calculatedMetrics.calculationMethod === 'manual_override' && '(Manual)'}
                {calculatedMetrics.calculationMethod === 'smart_segments' && '(Smart)'}
                {calculatedMetrics.calculationMethod === 'legacy_gaps' && '(Legacy)'}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-600">{calculatedMetrics.calculatedCurrentQuantity.toFixed(1)}m</div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-purple-600">{usageRecords.length}</div>
            </CardContent>
          </Card>
        </div>
          ...
          <Card className="mb-4">
            ...
            <CardContent>
              <div className="text-xs text-muted-foreground mb-2">
                Visual representation of cable usage (green = used, orange = waste, gray = remaining)
              </div>
              <div className="h-8 bg-gray-200 rounded relative overflow-hidden">
                {calculatedMetrics.usageSegments.map((segment, idx) => (
                  <div
                    ...
                    style={{
                      left: `${(segment.start / drumInfo.initial_quantity) * 100}%`,
                      width: `${(segment.length / drumInfo.initial_quantity) * 100}%`,
                    }}
                    title={`Used: ${segment.start}m - ${segment.end}m (${segment.length.toFixed(1)}m)`}
                  />
                ))}
                {calculatedMetrics.wastedSegments.map((segment, idx) => (
                  <div
                    ...
                    title={`Waste: ${segment.start}m - ${segment.end}m (${segment.length.toFixed(1)}m)`}
                  />
                ))}
              </div>
              ...
              {loading ? (
                <TableRow>
                  <TableCell colSpan={8} className="text-center py-8">
                    ...
                    <p className="text-muted-foreground mt-2">Loading usage records...</p>
                  </TableCell>
                </TableRow>
              ) : usageRecords.length > 0 ? (
                usageRecords.map((record, index) => {
                  const actualUsage = Math.abs((record.cable_end_point || 0) - (record.cable_start_point || 0))
                    ...
                    <TableRow key={record.id}>
                      ...
                      <TableCell>
                        <div className="text-xs">
                          <div>Start: {(record.cable_start_point || 0).toFixed(1)}m</div>
                          <div>End: {(record.cable_end_point || 0).toFixed(1)}m</div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <span className="font-medium text-blue-600">{actualUsage.toFixed(1)}m</span>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-xs text-muted-foreground">
                        {calculatedMetrics.calculationMethod === 'smart_segments' && 'Smart calculation'}
                        {calculatedMetrics.calculationMethod === 'legacy_gaps' && 'Legacy method'}
                        {calculatedMetrics.calculationMethod === 'manual_override' && 'Manual override'}
                      </TableCell>
                    </TableRow>
                  )
                })
        ...
        <Card className="mt-4">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm flex items-center gap-2">
              ...
              Calculation Method: {calculatedMetrics.calculationMethod.replace('_', ' ').toUpperCase()}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-sm text-blue-700 dark:text-blue-300 space-y-1">
              {calculatedMetrics.calculationMethod === 'smart_segments' && (
                <>
                  <p>• <strong>Smart Segments:</strong> Merges overlapping cable usage to calculate actual consumption</p>
                  <p>• <strong>Handles bidirectional usage:</strong> Accounts for cable being pulled from either end</p>
                  <p>• <strong>Waste calculation:</strong> Only counts truly unused cable segments as waste</p>
                </>
              )}
              {calculatedMetrics.calculationMethod === 'legacy_gaps' && (
                <>
                  <p>• <strong>Legacy Method:</strong> Calculates waste as gaps between consecutive usage points</p>
                  <p>• <strong>Chronological processing:</strong> Processes usage records in date order</p>
                  <p>• <strong>Gap-based waste:</strong> May overestimate waste with bidirectional usage</p>
                </>
              )}
              {calculatedMetrics.calculationMethod === 'manual_override' && (
                <>
                  <p>• <strong>Manual Override:</strong> Uses manually specified wastage value</p>
                  <p>• <strong>Direct control:</strong> Allows precise adjustment based on field knowledge</p>
                  <p>• <strong>Validation:</strong> Ensures total doesn't exceed drum capacity</p>
                </>
              )}
              <p>• <strong>Total deducted:</strong> {(calculatedMetrics.totalUsed + calculatedMetrics.totalWastage).toFixed(1)}m 
                ({(((calculatedMetrics.totalUsed + calculatedMetrics.totalWastage) / drumInfo.initial_quantity) * 100).toFixed(1)}% of capacity)
              </p>
            </div>
          </CardContent>
        </Card>
