    ...
    try {
      ...
      if (data) {
        setFormData({
          ...
          pricing_tiers: data.pricing_tiers || formData.pricing_tiers,
          bank_details: data.bank_details || formData.bank_details,
        })
      }
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[95vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Configure company information, pricing tiers, and bank details for invoices.
          </DialogDescription>
        </DialogHeader>
                ...
                {formData.pricing_tiers.map((tier, index) => (
                  <div key={index} className="grid grid-cols-4 gap-4 items-end">
                    ...
                    <div className="text-sm text-muted-foreground">
                      {tier.min_length}–{tier.max_length === 999999 ? "500+" : tier.max_length}m: PKR {tier.rate}
                    </div>
                  </div>
                ))}
              </div>
