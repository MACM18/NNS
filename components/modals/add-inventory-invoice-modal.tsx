...
export function AddInventoryInvoiceModal({ open, onOpenChange, onSuccess }: AddInventoryInvoiceModalProps) {
        ...
        if (i === index) {
          ...
          if (field === "item_id" && value) {
            ...
            if (selectedItem) {
              ...
              if (!selectedItem.name?.toLowerCase().includes("drop wire cable")) {
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Invoice items have been extracted. Please review and confirm.",
        variant: "default",
      })
    } catch (error: any) {
      toast({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validate items
      const validItems = invoiceItems.filter((item) => item.item_id && Number(item.quantity_issued) > 0)
      ...
      toast({
        title: "Success",
        description: `Invoice ${autoInvoiceNumber} created successfully`,
        variant: "default",
      })
  ...
  const handleInvoiceSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("inventory_invoices").insert({
    ...
    if (error) {
      console.error("Error adding inventory invoice:", error)
      toast({
        title: "Error",
        description: "Failed to add inventory invoice. Please try again.",
        variant: "destructive",
      })
    } else {
      toast({
        title: "Success",
        description: "Inventory invoice added successfully.",
      })
      onSuccess()
      setInvoiceNumber("")
      setSupplierName("")
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[95vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between">Invoice Items</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {invoiceItems.map((item, index) => {
                  ...
                  const isDropWireCable = selectedItem?.name?.toLowerCase().includes("drop wire cable")
                  return (
                    <div key={item.id} className="grid grid-cols-12 gap-2 items-end">
                      ...
                      <div className="col-span-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleItemChange(index, "quantity_requested", e.target.value === "" ? "" : e.target.value)
                          }
                        />
                      </div>
                      <div className="col-span-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            handleItemChange(index, "quantity_issued", e.target.value === "" ? "" : e.target.value)
                          }
                        />
                      </div>
                      <div className="col-span-1">
                      ...
                      {isDropWireCable && (
                        <div className="col-span-12 mt-2 flex gap-2 items-center">
                          ...
                          <Input
                            ...
                            onChange={(e) => handleItemChange(index, "drum_number", e.target.value)}
              ...
              <div className="mt-4 flex flex-row-reverse justify-start gap-4 items-center">
                ...
                {invoiceItems.length > 1 && (
                  <span className="text-sm text-muted-foreground">Click "Add Item" to create a new row</span>
                )}
              </div>
              ...
              <div className="text-sm text-muted-foreground">
                <p>
                  ...
                  {invoiceItems.filter((item) => item.item_id && Number(item.quantity_issued) > 0).length}
                </p>
                <p>
