  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.selected_drum_id && totalCableNeeded > 0) {
        ...
        const { data: lastUsage, error: lastUsageError } = await supabaseClient
          .from("drum_usage")
      ...
      const { data: lineDetails, error } = await supabaseClient
        .from("line_details")
        .insert([insertData])
        .select()
        .single()
      ...
      if (formData.selected_drum_id && totalCableNeeded > 0) {
        ...
        const { data: currentDrum, error: drumError } = await supabaseClient
          .from("drum_tracking")
        ...
        // Record drum usage
        await supabaseClient.from("drum_usage").insert([
          {
        ...
        const { data: dropWireItem } = await supabaseClient
      ...
      addNotification({
        title: "Success",
        message: `Telephone line details added successfully. ${
          calculatedWastage > 0 ? `Wastage: ${calculatedWastage.toFixed(2)}m` : ""
        }`,
        type: "success",
        category: "system",
      })
                        ...
                        {availableTasks.map((task) => (
                          <CommandItem key={task.id} value={task.id} onSelect={() => handleTaskSelection(task)}>
                            ...
                            <div className="flex flex-col">
                              <span className="font-medium">{task.telephone_no}</span>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-muted/50 rounded-lg">
              <div>
                ...
                <div className="text-lg font-bold text-blue-600">{formData.f1}m</div>
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    className="bg-blue-50 dark:bg-blue-950"
                  />
                  <p className="text-xs text-blue-600 mt-1">Auto: Same as Roll Plug ({formData.roll_plug})</p>
                </div>
              </div>
            </div>
