...
export function GenerateMonthlyInvoicesModal({ open, onOpenChange, onSuccess }: GenerateMonthlyInvoicesModalProps) {
  ...
  const calculateRate = (cableLength: number): number => {
    if (!companySettings?.pricing_tiers) {
  ...
  const generateInvoicePreviews = (lines: LineDetail[]) => {
    ...
    const totalAmount = lines.reduce((sum, line) => sum + calculateRate(line.total_calc), 0)

    // Generate invoice numbers
    const monthName = months.find((m) => m.value === selectedMonth)?.label || "Unknown"
    const baseInvoiceNumber = `S/Southern/HR/NC/${selectedYear.slice(-2)}/${monthName}`
    ...
    const previews: InvoicePreview[] = [
      {
        ...
        percentage: 90,
        lines: aLines,
        totalAmount: aLines.reduce((sum, line) => sum + calculateRate(line.total_calc), 0),
      ...
      },
      {
        ...
        percentage: 5,
        lines: bLines,
        totalAmount: bLines.reduce((sum, line) => sum + calculateRate(line.total_calc), 0),
      ...
      },
      {
        ...
        percentage: 5,
        lines: cLines,
        totalAmount: cLines.reduce((sum, line) => sum + calculateRate(line.total_calc), 0),
  ...
  const getInvoiceDate = (): string => {
    ...
    const nextYear = nextMonth > 12 ? Number.parseInt(selectedYear) + 1 : Number.parseInt(selectedYear)
  ...
  const handleGenerateInvoices = async () => {
    ...
    try {
      ...
      const jobMonth = `${months.find((m) => m.value === selectedMonth)?.label} ${selectedYear}`
      ...
      for (const preview of invoicePreviews) {
        ...
        const { error } = await supabase.from("generated_invoices").insert([invoiceData])
        if (error) throw error
      }

      addNotification({
        title: "Success",
        message: `Generated ${invoicePreviews.length} invoices for ${months.find((m) => m.value === selectedMonth)?.label} ${selectedYear}`,
        type: "success",
      })
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[95vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Generate 3 monthly invoices (A=90%, B=5%, C=5%) based on completed line installations.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    ...
                    <div className="text-2xl font-bold">{lineDetails.length}</div>
                  </div>
                  <div>
                    <Label className="text-sm text-muted-foreground">Total Amount</Label>
                    <div className="text-2xl font-bold">
                      PKR {lineDetails.reduce((sum, line) => sum + calculateRate(line.total_calc), 0).toLocaleString()}
                    </div>
                  </div>
                  <div>
                    <Label className="text-sm text-muted-foreground">Invoice Date</Label>
                    <div className="text-lg font-medium">{new Date(getInvoiceDate()).toLocaleDateString()}</div>
                  </div>
                  <div>
                    <Label className="text-sm text-muted-foreground">Job Month</Label>
                ...
                <Card key={preview.type}>
                  <CardHeader>
                    <CardTitle className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Badge variant={preview.type === "A" ? "default" : "secondary"}>Invoice {preview.type}</Badge>
                        <span className="text-sm text-muted-foreground">
                          {preview.percentage}% • {preview.lines.length} lines • PKR{" "}
                          {preview.totalAmount.toLocaleString()}
                        </span>
                      </div>
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => handlePreviewInvoice(preview)}>
                          <Eye className="h-4 w-4" />
                        </Button>
                        <Button size="sm" variant="outline" onClick={() => handleDownloadInvoice(preview)}>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      <div className="text-sm font-medium">Invoice Number: {preview.invoiceNumber}</div>
                      <Table>
                        <TableHeader>
                          <TableRow>
                        ...
                        <TableBody>
                          {preview.lines.slice(0, 5).map((line) => (
                            <TableRow key={line.id}>
                              ...
                              <TableCell>{line.total_calc.toFixed(2)}m</TableCell>
                              <TableCell>PKR {calculateRate(line.total_calc).toLocaleString()}</TableCell>
                              <TableCell>PKR {calculateRate(line.total_calc).toLocaleString()}</TableCell>
                            </TableRow>
                          ))}
          ...
          {!loading && lineDetails.length === 0 && selectedMonth && selectedYear && (
            <div className="text-center py-8">
              <FileText className="h-12 w-12 mx-auto mb-4 opacity-50" />
              <p className="text-muted-foreground">
                No completed line installations found for {months.find((m) => m.value === selectedMonth)?.label}{" "}
                {selectedYear}
              </p>
            </div>
          )}
